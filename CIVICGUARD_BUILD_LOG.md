...
| D31 | 5 Aug | DSAR | Days Left badge reads "1 days left" rather than "1 day left" | Correct singular form | Minor | **Closed 9 Aug** | Ch4 screenshots |
...
The Related Work Review was the weakest section proportionally, and in the dissertation the literature review carries **0%** and transfers unchanged. The weakest area is not marked again.

Implementation, testing and validation carries **20%**, the largest single block, and is precisely what the 5 to 9 August work produced: 49 logged defects, a 23-item test inventory, live before-and-after evidence, and the F1 cross-cutting finding.

### Supervisor requests to action in the dissertation

1. **Numbered headings throughout.** Explicitly requested for Milestone 3. Report structure carries 5%. Formatting decision, no analytical cost.
2. **Benchmark existing efforts to monitor GDPR via IT.** Requested against the introduction. Satisfied by the OneTrust and Keepabl comparison already planned under DD4.
3. **Summary tables in the style of Table 1 and Table 2.** Apply to the defect log and to the five scenario results in Ch4.
4. **Link the design to the literature review.** The highest-value request. Ch4 design carries 15%, and the requirement is to show each module answering a failure the literature identified. The build log already supports this: F1 connects the testing findings to the ICO evidence base, and the five scenarios connect each module to a documented enforcement case.

### Risk

The viva carries 20% on its own, weighted 40% cohesion of argument, 40% questions and answers, 20% presentation. This cannot be prepared on 10 August and is the largest single unmitigated risk to the distinction. It is a further reason to protect the scenario replays over the capture session if 10 August runs short, since the replays are the substance of what will be asked.

---

## STATUS AT CLOSE OF 9 AUGUST

**All thirteen blockers identified during the 5 August test pass are closed.** Both evaluation risks are resolved: the DPP Law Ltd scenario (R1) and the Lewisham scenario (R3) can now be replayed and will produce the correct result.

Defects closed 7 to 9 August: D4, D8, D9, D11, D12, D14, D16, D17, D18, D21, D22, D24, D27, D28, D31, D33, D34, D35, D36, D37, D38, D39, D48. D10 withdrawn as incorrectly recorded.

**Remaining open, none blocking:** D2 (disclosure form lacks recipient and lawful basis), D43 and D45 (future dates accepted), D30 (unstable row ordering), D41 and D42 (filter state invisible), D7, D19, D26, D32, D40, plus the test-data records D5, D6, D15, D29, D44, D46, D47, D49 which clear at the re-seed.

**Scoped out and to be documented rather than built:** the two-month extension and stop-the-clock provisions (D23), deadline escalation (DD2), the compliance summary view (DD3), server-side field validation (DD6), and breach lifecycle beyond notification (DD8).

**Remaining before departure on 11 August:** re-seed with clean scenario data, code freeze and tag, systematic capture session, five ICO scenario replays, evidence pack, supervisor email.
...
| File | Shows | Status |
|---|---|---|
| `before_D16_dsar_30day_05aug.png` | DSAR list, 5 August. Four faults in one frame: the "30-day statutory deadline" subtitle (D16, D24); record 5, received 2026-01-31, deadline 2026-03-02, two days granted with no legal basis (E9); record 4, deadline 2026-07-03, completed 2026-07-13, ten days late, displaying a green "Done" badge (D18, evidences the Lewisham risk R3); On Track card reading 0 against five records (D19). The "before" half of the DSAR rebuild pair, against `after_D48_dsar_cards_09aug.png`. Stronger than the dashboard pair, since the fault and its correction are both legible without a caption | **Saved 5 Aug** |
...
| `after_D48_dsar_cards_09aug.png` | DSAR Manager with five summary cards agreeing with the row badges, and the corrected Art. 12A subtitle. Three overdue, one completed on time, three completed late | **To save 9 Aug** |
...
## I. SESSIONS AFTER THE FREEZE

Appended from `CHANGE_LEDGER_10AUG.md`, which holds the detail. This section
exists so the build log remains a complete record on its own.

### 18 August, re-seed executed

`seed_data.py` replaced in full and run. Written 10 August, executed 18 August:
the freeze day ran short and the build environment was unavailable 11 to 17
August. The old seed carried em dashes in all three modules, thirty-day
deadline arithmetic and eight test records. The replacement clears
`disclosures`, `breaches`, `dsars` and `audit_log`, leaves `users` intact, and
loads a background caseload exercising every status band: six disclosures, five
breaches, seven DSARs.

The seed replicates `add_calendar_month` and `calculate_relevant_time` from
`app.py` rather than using `timedelta(days=30)`, and prints every record with
its computed deadline. This gives two independent calculations of the same
value, so the interface can be checked against a computed expectation rather
than inspected. Output retained as `seed_verification_18aug.txt` and `.png`.

**Verification against the interface, 18 and 23 August.** All seven DSAR
deadlines match. T. Ferreira renders 2026-01-31 to 2026-02-28, confirming D16
closed in the application and not only in the seed, and is flagged "Completed
late" against a completion date of 2026-04-16, confirming D18 closed and
evaluation risk R3 cleared. The Revenues mailbox breach renders "240h to
notify, LATE" in red despite ICO Notified reading Yes, confirming D8 closed and
risk R2 cleared. Dashboard risk strip reads 1 / 1 / 2 / 1 and reconciles
against the module rows.

**Three discrepancies found and recorded, none blocking.**

The seed's own "Expected card states" line states the Breach Tracker should
show 2 overdue. It should read 1. That line is hardcoded; the per-record lines
above it are computed and correct. The application is right and the summary
line is wrong. Left uncorrected in the script so that the verification file
remains the artefact actually produced on the day.

The Breach Tracker's four cards total four against five rows: the 14h planning
objection sits in no card. This differs from the DSAR module, whose five cards
were reconciled to cover every row under D48. The breach cards surface risk
rather than count records, which is defensible, but the inconsistency is
visible on screen and should be stated in Ch4 as a choice.

Days-left figures differ from the printout by one throughout. The seed
subtracts whole dates; `app.py` compares the deadline against the current time,
so a deadline five days out at 15:39 renders as four full days remaining. The
deadline dates themselves agree exactly, which is where the legal obligation
sits, and the drift is conservative. **Not a recurrence of F1.** Ch4 states
that "days left" means complete days remaining.

### 23 August, Chapter 4 figure set

Evidence folder reorganised into `defects/`, `ch4_figures/` and `scenarios/`.
Filenames referenced throughout this log now sit one level down in `defects/`;
references are descriptive rather than links, so nothing is broken, but a
reader following one should look there.
`after_D18_late_completion_flagged_08aug` converted from `.webp` to `.png`.

Ten figures captured to `ch4_figures/`: DPO dashboard, all three entry forms,
login screen, approval flow, rejection prompt and result, and the Officer view
of both dashboard and disclosure list.

The dashboard was captured before any approval action, while both disclosures
were still Pending, so the volume cards read 2 / 3 / 5.

**Two genuine audit entries created.** Disclosure 2 approved and disclosure 1
rejected through the interface as DPO, writing `audit_log` rows with real
timestamps. This matters because the four audit rows loaded by the seed were
written by a script, not by an officer, and cannot evidence that the mechanism
captures decisions. These two can.

**Known limitation accepted, not fixed.** The rejection reason is collected by
a browser `window.prompt()` dialog rather than a form field, so the figure
shows a native dialog labelled with the origin `127.0.0.1:5000`. Replacing it
with a modal was considered and declined: the change would touch the rejection
route, which is the audit-writing path closed as D37 and D39, and function is
already correct. Changing it after the scenario replays would mean the code
producing the Ch5 evidence is not the code submitted, so the only clean options
were before the replays or not at all. Write up alongside D2 and DD6 as a
scoped limitation. Three points to state: the reason is collected outside the
application's own interface; "mandatory" holds in the sense that Cancel
abandons the action entirely; and the local address in the figure is consistent
with a prototype on a development server.

**Data state after this session.** Pending Disclosures now reads 0. Any figure
needing a pending record must be captured before further approvals. Re-seeding
